...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None,timage=None):
        """Return updated labels and image with added border."""
        if labels is None:
            labels = {}
        img = labels.get('img') if image is None else image
        timg = labels.get('timg') if image is None else image
        ...
        if shape[::-1] != new_unpad:  # resize
            ...
            timg = cv2.resize(timg, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
        img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                 value=(114, 114, 114))  # add border
        timg = cv2.copyMakeBorder(timg, top, bottom, left, right, cv2.BORDER_CONSTANT,
                                 value=(114, 114, 114))  # add border
        ...
        if len(labels):
            ...
            return labels  # from this return
        else:
            return img
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            tim_new = np.zeros(tim.shape, np.uint8)
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                ...
                cv2.drawContours(tim_new, instances.segments[[j]].astype(np.int32), -1, (1, 1, 1), cv2.FILLED)

            result = cv2.flip(im, 1)  # augment segments (flip left-right)
            tresult = cv2.flip(tim, 1)
