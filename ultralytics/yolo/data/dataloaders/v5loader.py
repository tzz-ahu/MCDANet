...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if False:
            ...
            # Load image
            img, img1, timg, timg1,(h0, w0), (h, w) = self.load_image(index)
            ...
            img, img1,timg,timg1, ratio, pad = letterbox(img,img1, timg,timg1,shape, auto=False, scaleup=self.augment)

            shapes = (h0, w0), ((h / h0, w / w0), pad)  # for COCO mAP rescaling
            # if index == 7063 or index == 7064 or index == 2384 or index == 2385:
            ...
            if self.augment:
                ...
                img,img1,timg,timg1, labels = random_perspective(img,img1,
                                                      timg,
                                                 timg1,
                                                 labels,
                                                 degrees=hyp['degrees'],
        ...
        if self.augment:
            # Albumentations
            img,img1,timg,timg1, labels = self.albumentations(img,img1,timg,timg1, labels)


            #cutswap for rgb and thermal images
            #if random.random() < 0.5:
            #    x = np.random.randint(0, 640)
            #    y = np.random.randint(0, 640)
            #    w = np.random.randint(0, 640)
             #   h = np.random.randint(0, 640)
            #    img, timg = self.cutswap(img, timg, x, y, w, h)
            # if random.random() < 0.5:
            #     x = np.random.randint(0, 640)
            #     y = np.random.randint(0, 640)
            #     w = np.random.randint(0, 640)
            #     h = np.random.randint(0, 640)
            #     img1, timg1 = self.cutswap(img1, timg1, x, y, w, h)


            nl = len(labels)  # update after albumentations

            # HSV color-space
            augment_hsv(img,img1,timg1, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])  # only for rgb images
        ...
        timg = timg.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        timg = np.ascontiguousarray(timg)
        timg1 = timg1.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        timg1 = np.ascontiguousarray(timg1)

        return torch.from_numpy(img),torch.from_numpy(img1),torch.from_numpy(timg),torch.from_numpy(timg1), labels_out, self.im_files[index], shapes

    def cutswap(self,image1, image2, x, y, width, height):
        x = max(x, 0)
        y = max(y, 0)
        x_end = min(x + width, image1.shape[1])
        y_end = min(y + height, image1.shape[0])
        # 使用Numpy切片来交换两幅图像的像素
        temp = np.copy(image1[y:y_end, x:x_end])
        image1[y:y_end, x:x_end] = image2[y:y_end, x:x_end]
        image2[y:y_end, x:x_end] = temp
    ...
    def load_image(self, i):
        ...
        name = f.split('frame')[1].split('.')[0]
        
        match = re.search(r'frame(\d+)\.jpg$', f)
        current_idx = int(match.group(1)) if match else -1
        
        seq_dir = os.path.dirname(f)
        
        if not hasattr(self, '_seq_frame_cache'):
            self._seq_frame_cache = {}
        
        if seq_dir not in self._seq_frame_cache:
            import glob
            all_jpgs = glob.glob(os.path.join(seq_dir, 'frame*.jpg'))
            indices = []
            for jpg in all_jpgs:
                m = re.search(r'frame(\d+)\.jpg$', os.path.basename(jpg))
                if m:
                    indices.append(int(m.group(1)))
            indices.sort()
            self._seq_frame_cache[seq_dir] = indices
        else:
            indices = self._seq_frame_cache[seq_dir]
        ...
        f_target = os.path.join(seq_dir, f'frame{idx1}.jpg')
        f1_target = os.path.join(seq_dir, f'frame{idx2}.jpg')
        ...
        if name == '0':
            ...
            name = int(name)-1
            f1 = f.split('frame')[0]+'frame'+str(name)+'.jpg'
            if name == 0:
                f2 = f1
                f3 = f2
            else:
                name = name - 1
                f2 = f.split('frame')[0]+'frame'+str(name)+'.jpg'
                if name ==0:
                    f3 = f2
                else:
                    name = int(name)-1
                    f3 = f.split('frame')[0]+'frame'+str(name)+'.jpg'
        if True:  # not cached in RAM
            if fn.exists():  # load npy
                im = np.load(fn)
            else:  # read image
                im = cv2.imread(f)  # BGR
                im1 = cv2.imread(f_target) # f1
                timg = cv2.imread(f.replace("RGB","T"))
                img2 = cv2.imread(f1_target) #f2
                #print(f,f1)
                assert im is not None, f'Image Not Found {f}'
            h0, w0 = im.shape[:2]  # orig hw
            #cv2.imwrite('orignal-rgb.jpg', im)
            #cv2.imwrite('orignal-t.jpg', timg)
            r = self.img_size / max(h0, w0)  # ratio
            if r != 1:  # if sizes are not equal
                interp = cv2.INTER_LINEAR if (self.augment or r > 1) else cv2.INTER_AREA
                im = cv2.resize(im, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=interp)
                timg = cv2.resize(timg, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=interp)
                im1 = cv2.resize(im1, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=interp)
                img2 = cv2.resize(img2, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=interp)

            return im,im1,timg,img2, (h0, w0), im.shape[:2]  # im, hw_original, hw_resized
    ...
    def collate_fn(batch):
        ...
        im, img1,timg,timg1, label, path, shapes = zip(*batch)  # transposed
        ...
        return {
            ...
            'timg': torch.stack(timg,0),
            'timg1': torch.stack(timg1,0),
            'cls': cls,
            'bboxes': bboxes,
