...
class BasePredictor:
    ...
    def stream_inference(self, source=None, model=None):
        ...
        for minibatch in self.dataset:
            lis.append(minibatch[1])
        #image = np.concatenate([list[0],list[1],list[2]],2)
        for batch in self.dataset:
            ...
            # Preprocess
            with profilers[0]:
                #a = self.preprocess(lis[0])
                #b = self.preprocess(lis[1])
                #c = self.preprocess(lis[2])
                im = self.preprocess(im0s)
                #ims = torch.cat([a, b, c], 1)
