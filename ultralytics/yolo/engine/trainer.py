...
from ultralytics.yolo.utils.dist import ddp_cleanup, generate_ddp_command,get_model_info
...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        print("Model Summary: {}".format(get_model_info(self.model, (640, 640))))
        #batch_size = 1
        #input_shape = (batch_size, 3, 640, 640)
       
        #flops, macs, params = calculate_flops(
        #          model=self.model,              # 你的PyTorch模型
        #          input_shape=input_shape,  # 输入形状
        #          output_as_string=True,    # 以字符串形式返回（如"1.23 GFLOPS"）
        #          output_precision=4,
        #          print_detailed=False        # 自动打印每一层的FLOPs详情，用于调试
         #   )
        ...
        with torch.cuda.device(0):
            flops, params = get_model_complexity_info(self.model, (3, 640, 640), as_strings=True,
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    for module in actual_model.modules():
                      if hasattr(module, '_last_alpha') and module._last_alpha is not None:
                        alpha_mean = module._last_alpha.mean().item()
                        break  
                    if not np.isnan(alpha_mean):
                      self.all_alpha_values.append(alpha_mean)'''
            ...
            '''if self.all_alpha_values:
              import matplotlib
              matplotlib.use('Agg')
              import matplotlib.pyplot as plt
    
              plt.figure(figsize=(12, 4))
              plt.plot(self.all_alpha_values, 'b-', linewidth=0.5, marker='o', markersize=0.5)
              plt.xlabel('Batch index (cumulative)')
              plt.ylabel('Alpha (batch mean)')
              plt.title(f'Alpha variation (up to epoch {epoch+1})')
              plt.grid(True)
              plt.tight_layout()
              plt.savefig('alpha_curve.png', dpi=150)  
              plt.close()
    
              LOGGER.info(f"Updated alpha curve saved to alpha_curve.png (total batches: {len(self.all_alpha_values)})")
              '''
